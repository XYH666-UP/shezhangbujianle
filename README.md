...
很开心过了这么长时间，这个项目还能持续受到大家关注。上海疫情封控开始后，我转而尝试应用该类型技术支持疫情封控下的居委会志愿者；封控结束后，我和一些小伙伴继续致力于尝试应用相关技术在基层政务治理方面，几个月来这几乎耗费了我全部的业余时间（因为白天还要上班搬砖……） ，所以很长一段时间以来我也没时间更新这个repo，也再未开放体验活动。因此，请大家暂时**不要**添加这个机器人账号了。

🔥🔥🔥 【2024.4.24】

感谢大家的关注，全新的Awada项目已经立项，致力于打造拥有目的意识和类人脑自主决策能力的bot，详见 [Awada Github Repo](https://github.com/TeamWiseFlow/awada)
...
无尽的。在这里我们用到了百度飞桨@PaddlePaddle 发布的预训练模型—— [simnet_bow](https://www.paddlepaddle.org.cn/hubdetail?name=simnet_bow&en_category=SemanticModel) ，它能够自动计算短语相似度，基于百度海量搜索数据预训练，实际应用下来效果非常不错，且运算速度快，显存占用低。这里不得不特别表扬下百度飞桨，尤其是其中的预训练模型开放库paddlehub，里面有不少非常实用的“小模型”，均可“开箱即用”。
...
好到什么程度呢？这么说吧，好到了我们直接拿来用的程度 :joy:……事实上，本项目代码库中的__init__.py、inspurai.py、url_config.py这三个文件都直接来自 [浪潮的开源代码](https://github.com/Shawn-Inspur/Yuan-1.0/tree/main/yuan_api) 。
...
不过囿于人力等因素限制，本项目现阶段的example语料库还是比较"单薄"的，还存在很大的填充空间。在目前体量的语料库支撑下，公测中大部分对话问题的simnet_bow相似度得分都在0.7~0.8，而实际上0.85以上得分才是意思比较相近的两句话。
，且simnet_bow计算出的相似度也有一定概率是不准的，即两个短语相似度得分达到0.9以上了，但实际上意思还是差的比较远的。

所以为了能够获得更好的回复效果，就可能需要更加丰富、颗粒度更加细腻的语料，这就需要编导能够预设更多问题，对于持续运营的项目而言，可能建立常态化的语料文件维护机制是必要的。

另外对于要求回复比较固定的问题，比如人物的性别、姓名这些，最好可以以不同的提问措辞冗余准备3~5组问答example。这倒不是说源大模型或者simnet_bow没有"举一反三"的能力，而是我们发现在实践中，one-shot情况下，模型仅能get回答的范围，而无法保证
严格遵守example的答案设定。举个例子，example为："Q：你的性别 A：女生"，用户提问为"你是男生还是女生啊？"，在仅提供这一条example的情况下，模型既可能回答"男生"，也可能回答"女生"，甚至回答"你猜"，但不会回答"我是人"这种……
如果给定三条example的话，并且example 的答案都是"女生"，那么模型绝大部分情况下都会生成"女生"……

另外，对于答案要求唯一的对话内容，甚至也可以考虑直接程序写死，或者采用传统的NLU方式提取关键词，匹配词槽的方案。

最后，我们在源升级版dialog模型的应用中还发现，对于zero-shot生成，如果用户问题是"好"、"好的"这类比较无意义的提问，源有一定的概率会给出"帮您安排退款"、"请您下单"……之类的回复（同样的情况在wudao的Eva中也有），这可能是
中文大模型的训练语料来源确实比英文要局限很多的原因所造成的，所以我们建议，可以针对这类无意义提问程序上设定不生成回复，或者进行固定格式的回复。

总之，对于"目的型对话"，因为其具体要求远远大于开放域对话，所以虽然可以采用基于大模型的端到端方案作为主体，但也要综合混用其他方案。另外会非常依赖example语料的质量和丰富度，这方面的心得体会请见下文第四部分。

# 记忆机制

本来这个项目一开始是不打算引入记忆机制的，因为我们看源1.0在合适example的few-shot下生成效果已经很不错了，但在之后的测试中我们发现，会有用户习惯先提问再@AI回答，或者私聊中先发一句问题，然后再另发一句"你对这个问题的看法？"；另外我们也发现AI如果不记忆自己之前答案的话，
后续生成的结果会比较缺乏连续性，甚至给出前后矛盾的回答！这些问题迫使我们决定增加"多轮对话记忆机制"。

原理很简单，就是把之前若干轮次用户与AI的对话存在一个列表里面，然后提交生成的时候把这个列表和当前问题文本join一下，当然具体实施的时候，我们需要调整下提交的pre-fix和输出的pre-fix这些……
实践发现增加记忆机制后，AI因为生成依据变多，明显弥补了其逻辑能力的短板，如下图是我们的一段测试对话，AI表现出了一定"逻辑推理能力"：

![img](https://github.com/bigbrother666sh/shezhangbujianle/blob/main/assets/a5xrh-20blg.jpeg)

【update】
但在随后的应用中，我们发现：不管我们设定程序记忆几轮对话，貌似 *起作用比较大的只有最后一轮*，如下图

![img](assets/Collage_20220508_202242.jpg)

从右到左分别是让程序只记忆最后一轮对话、记忆过去两轮对话以及单边记忆过去四轮对话（只记忆用户提问，不记忆自己的回答），可见三个模式下，程序都能对第一次反转产生很好的应答，但对于第二次反转就无能为力了……

另外我们还发现，对于example语料匹配度比较低的提问，过长的记忆机制**反而**可能让生成效果下降，最典型的情况是造成"重复回答"，即AI会在后续轮次的对话中一直重复之前已经说过的话……

其实仔细想想这也不难理解，因为源1.0的API机制其实是把example加到待输入文本前作为最终提交的输入，而我们设计的记忆机制也是把记忆中的文本加到待输入文本前，这样的话，其实是记忆文本和example语料共同对生成结果施加影响，
对于example语料匹配度比较低的提问，example影响力低，那么相对而言过去轮次对话的影响力就高了……所以对于这个问题，*终极的解决方案也是尽可能提高example语料库的质量和丰富度*。

但是如果我们采用单边记忆模式（只记忆用户提问，不记忆自己的回答），虽然对生成效果没有不良影响，但实际场景中，会有大量用户提问是基于AI上一轮回复内容的情况，这个时候单边记忆由于舍弃了回答文本，就无法保证回答的连续性了……

在后续"蔡晓问答"版本程序中（caixiao.py)我们额外增加了一个校验机制，如果生成结果等同于上一轮回答则会让AI重新生成一次……实际应用中，这个机制确实可以一定程度上降低重复回答的发生。而对于用户明确提问"你刚才说啥了"这类的情况，源1.0
在重复几轮生成后，也会生成一个跟原来语句答案非常类似但不完全一样的回答（比如增加或者减少一个标点符号）。

另外，现在源也发布了升级版模型，我们也第一时间升级了新版的对话模型（dialog），有如下2点心得：

1、新版dialog模型增加了不少常识性知识的预训练，所以开放域对话任务的zero-shot效果得到了大幅提高；

2、但记忆机制的效果跟原模型（base-10b）还是一样的，即不管记忆几轮，主要起作用的还是最后一轮。**但如果**你问它“上上一轮你说的是啥”这样（甚至更遥远），模型大部分时间也能够正确回答（这点倒是很神奇）。

总结来看，开放域对话可以放大记忆轮次，但目的性对话，记忆轮次太多会影响example的效果，导致生成结果更容易偏离预期。

更理想的方案可能需要对记忆机制进行更加复杂的编码，能够自动提取
关键信息（也许seq2seq模型是个好主意?)，只记忆关键信息和上一轮对话内容……当然记忆机制本身是一个比较复杂的话题，尤其对于目的性对话任务，所以欢迎大家在这方面与我共同探讨，并探索更加有效的方案！

# 导演机制
我们还必须澄清一点，现阶段的任何应用你都不能指望AI单独去完成，它依然需要人类的协助，或者监督、或者提示、或者backup……

在本项目上，我们为AI设定的辅助机制就是——**导演**。他的作用是在出现AI不适用的情况下对AI进行"手动引导"，这些情况有一些是游戏机制本身，有一些是因为目前我们采用的wechaty puppet-xp协议限制（ 有关wechaty和puppet协议的关系可以查看 [wechaty主页](https://github.com/wechaty) ）。
...
另外，针对上一轮不好的生成结果有可能影响后续轮次生成的情况，我们也设计了"记忆遗忘机制"，导演可以随时让程序遗忘其对某个用户或者群聊的记忆，这也是应急机制的一部分。
### 流程引导
...
在测试的后期，我们还发现输入文本中如果包含#和&会导致源1.0API报错(issue详见 [这里](https://github.com/Shawn-Inspur/Yuan-1.0/issues/13) ）, 浪潮团队建议我们进行转义替换，但我们觉得替换为"号"和"和"字可能会更贴近用户的真实意图……
...
<img alt="img" height="600" src="https://github.com/bigbrother666sh/shezhangbujianle/blob/main/assets/WechatIMG337.jpeg" width="768"/>
...
<img alt="img" height="330" src="https://github.com/bigbrother666sh/shezhangbujianle/blob/main/assets/WechatIMG334.jpeg" width="768"/>
...
除综合使用了浪潮源1.0大模型和百度飞桨的paddlehub预训练模型外，本项目涉及到的另一个关键组件是wechaty，这是一个可以几乎无缝对接包括微信在内一众主流IM软件的“神器”，
...
好比我写代码时最爱听一首歌在网易云音乐上的热评——又能听林忆莲，又能抖腿，你们还要啥？

不过也不得不说的是，puppet-xp目前的功能实在有限，仅能实现文本信息的收发，这导致了很多限制，甚至留下了一些硬伤，比如上文说到的引用消息问题，以及多端同步信息错误识别问题等……
好在puppet-xp团队也在不停的升级开发，我们期待后续版本可以在免费的同时提供更多功能支持。现在NLP大模型都有往多模态发展的趋势，text2img甚至text2video都已经在路上，这些如果有朝一日能够与Wechaty结合一定会催生出更多有趣的玩法。

然而在这里，还是要向 **@Wechaty** 社区致敬！”好用到哭“——你们对得起这个评价！ 也向爱写诗的**laozhang**（puppet-xp作者）致敬！感谢你把这么好的东西无私的奉献了出来！

致敬**浪潮源1.0**团队、**百度飞桨**团队，致敬**github**和**csdn**，本项目实现过程中没少在这两个地方抄代码（其实不止本项目了，我想每个程序员都一样吧），总之致敬**开源社区**，致敬所有**开发者**！

最后向**Linus Torvalds**表达我由衷的敬佩！尤其当我因为本项目而系统的学习了git和github的历史之后，我更加要表达这种敬佩，这确实是个凭一己之力改变世界的男人！
...
特别感谢华东理工大学图书馆的**吉久明老师**
授予我们原作的改编权，同时还无偿贡献了配套的心理测验题目，通过16道是否题，就可以比较精确的匹配到跟玩家性格特征最贴近的角色，从而让整个游戏的沉浸感更加强烈，个人认为，这倒是每一个剧本杀都值得引入的一个机制。

# 写在最后的话

做这个项目是有感于去年大热的各种虚拟人，我丝毫不怀疑这个赛道的潜在价值，虽然我们目前还没有办法统一而清晰的定义作为人类最终归宿的“元宇宙”，但有一点是毋庸置疑的，即未来的元宇宙中，
虚拟人数量将数倍于真人，因为只有这样，才能让我们每个人过得比现实世界中更好。然而目前阶段，虚拟人在“好看的皮囊”方面可谓日新月异，然而“有趣的灵魂”方面还都很欠缺，
靠“中之人”驱动毕竟不是长久之策；另一方面，自去年上半年我了解到NLP领域近两年来在生成式预训练大模型方面的长足进展后，也一直想看看基于这种大模型有什么可以实际落地的场景，
就这样，两个不同角度的想法合流成为了本项目的初衷。

最后的最后，我们真心欢迎各路高手和专业人士在本开源项目基础上不断增加新的剧情、新的玩法，也期待更多的中文NLP大模型研究机构移植本项目，如果有一天能够看到各路AI在一起打剧本杀，应该会很有意思！
